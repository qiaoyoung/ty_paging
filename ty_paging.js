...
function ty_dealPaging(totalPage, callback) {
  ...
  if (maxPage > 1) {
    ...
    $('#ty_paging .pageIndex').each(function (index, liDom) {
      ...
      if (ty_currentPage == $(liDom).val()) { //选中分页修改css
        $(liDom).css('backgroundColor', '#31C27C').css('color', 'white').css('border', '0px');
      } else {
        $(liDom).css('backgroundColor', 'white').css('color', '#666666').css('border', "1px solid #ddd");
      }
    });
